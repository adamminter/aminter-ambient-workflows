...
def move_issue(issue_key, target_project, target_type_name):
    ...
    r = requests.put(
        f"{JIRA_URL}/rest/api/3/issue/{issue_key}",
        json=body, auth=AUTH, headers=HEADERS, timeout=30,
    )
...
def cmd_migrate(board_id=None, jql=None, team_name=None, mode="lazy",
                ...
                sprint_board_id=None, sprint_count=None):
    ...
    # Build sprint label map if requested
    sprint_map = {}
    if sprint_labels and (sprint_board_id or board_id):
        sid = sprint_board_id or board_id
        sprints = get_board_sprints(sid)
        if sprint_count:
        ...
        for sprint in sprints:
            ...
            for key in sprint_issues:
                if key not in sprint_map:
                    sprint_map[key] = []
                sprint_map[key].append(label)
        print(f"Sprint labels prepared for {len(sprint_map)} issues across {len(sprints)} sprints")
    ...
    for i, issue in enumerate(to_migrate):
        ...
        # Sprint labels
        extra_labels = sprint_map.get(key, [])

        if dry_run:
            print(f"  [{i+1:4d}/{len(to_migrate)}] {key:12s} | {src_type:15s} -> {target_type:15s} | {src_status:15s} -> {target_status:12s} | {summary}")
            if src_comps != target_comps:
                print(f"               components: {src_comps} -> {target_comps}")
            if extra_labels:
                print(f"               sprint labels: {extra_labels}")
            migrated.append(key)
            continue
        ...
        if not ok:
            print(f"  [{i+1:4d}/{len(to_migrate)}] {key:12s} | FAILED to move: {err}")
        ...
        if not ok:
            print(f"  [{i+1:4d}/{len(to_migrate)}] {new_key:12s} | Moved but failed to set Team: {err}")
        ...
        print(f"  [{i+1:4d}/{len(to_migrate)}] {new_key:12s}{old_key_display} | {target_type:15s} | {target_status:12s} | {summary}")
    ...
    log_data = {
        ...
        "failed": failed,
        "sprint_labels_applied": bool(sprint_map),
        "sprint_count": len(sprint_map) if sprint_map else 0,
...
def main():
    ...
    parser.add_argument("--sprint-labels", action="store_true",
                       help="Preserve sprint names as labels")
